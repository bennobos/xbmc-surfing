...
class Main:
    def __init__(self):
        ...
        # Retrieve the event detail, including rounds
        api = SurfingAPI()
        event = api.get(url)
        for r in event['rounds']:
            detail_url = '/rounds/' + str(r['id'])
            parameters = {"action" : "list_heats", "plugin_category" : r['name'], "url" : detail_url, "next_page_possible": "True"}
            url = sys.argv[0] + '?' + urllib.urlencode(parameters)
            listitem = xbmcgui.ListItem(r['name'], iconImage="DefaultFolder.png")
